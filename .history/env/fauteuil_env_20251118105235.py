    ...
    def __init__(self, config):
        ...
        for group in self.config.get('static_groups', []):
            ...
            for i in range(count):
                ...
                # Calcule l'orientation (converge/diverge)
                ...
                # Ajoute l'humain statique à la liste
                self.humans.append({
                    "type": "static", # <-- Marqueur important
                    "pos": human_pos,
                    "direction": self.normalize(direction_vec),
                })

        # B. Génère les HUMAINS DYNAMIQUES (depuis la config)
        ...
        for _ in range(dynamic_count):
            ...
            while not valid and attempts < 100:
                ...
                # Vérifie la distance avec TOUS les humains déjà créés
                # (inclut les groupes statiques et les dynamiques précédents)
            ...
            self.humans.append({
                "type": "dynamic", # <-- Marqueur important
                "pos": pos,
