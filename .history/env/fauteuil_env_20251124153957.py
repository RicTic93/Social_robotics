...
class FauteuilEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        partial_obsss = self._get_partial_obs(full_obs)
    ...
    def step(self, action):
        print(f"Action: {action}, Robot Pos Before: {self.robot_pos}")
        self.robot_pos += action * self.max_speed
        self.robot_pos = np.clip(self.robot_pos, 0, 10)
        print(f"Robot Pos After: {self.robot_pos}")
        self.move_humans()
        reward = 0
        terminated = False
        ...
        for obj in self.objects:
            if np.linalg.norm(self.robot_pos - obj["pos"]) < 0.5 + obj["radius"]:
                ...
                terminated = False
                self.regenerate_layout = False
                print(f"Collision with object at {obj['pos']}, Reward: {reward}")
                break
        ...
        if not terminated:
            for human in self.humans:
                if np.linalg.norm(self.robot_pos - human["pos"]) < 0.5:
                    ...
                    terminated = False
                    self.regenerate_layout = False
                    print(f"Collision with human at {human['pos']}, Reward: {reward}")
                    break

        # Pénalité si entre deux humains convergents
        if self.is_between_converging_humans():
            reward += -5.0
            print(f"Between converging humans, Reward: {reward}")
        ...
        print(f"Final Reward: {reward}, Human Feedback: {human_feedback}")
        full_obs = self._get_obs()
