...
class FauteuilEnv(gym.Env):
    ...
    def is_between_converging_humans(self):
        for i, h1 in enumerate(self.humans):
            for j, h2 in enumerate(self.humans):
                if i >= j:
                    continue
                # Vérifie si les deux humains sont convergents et si le fauteuil est entre eux
                if h1["type"] == "static" and h2["type"] == "static":
                    d1 = np.linalg.norm(self.robot_pos - h1["pos"])
                    d2 = np.linalg.norm(self.robot_pos - h2["pos"])
                    d_humans = np.linalg.norm(h1["pos"] - h2["pos"])
                    # Si la somme des distances fauteuil-humain ≈ distance entre humains, le fauteuil est entre eux
    ...
    def render(self):
        ...
        pygame.draw.circle(
            self.screen,
            (0, 255, 0),
            (int(self.goal_pos[0] * 50) + 25, int(self.goal_pos[1] * 50) + 25), # goal position scaled
            15  # radius in pixels
        )
