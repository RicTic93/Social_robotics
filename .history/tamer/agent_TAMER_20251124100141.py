...
class SGDFunctionApproximator:
    ...
    def _get_partial_obs(self, full_obs, robot_pos, goal_pos, objects, humans, max_objects_in_view, max_humans_in_view):
        ...
        for obj in objects:
            if self._is_in_field_of_view(robot_pos, goal_pos, obj["pos"]):
                obj_features.extend(obj["pos"])
                obj_features.append(obj["radius"])
        obj_features += [0.0] * (3 * (max_objects_in_view - len(obj_features) // 3))

        # Ajoute les humains visibles
        human_features = []
        for human in humans:
            if self._is_in_field_of_view(robot_pos, goal_pos, human["pos"]):
                human_features.extend(human["pos"])
        human_features += [0.0] * (2 * (max_humans_in_view - len(human_features) // 2))

        partial_obs = np.concatenate([partial_obs, obj_features, human_features])
        return partial_obs
...
class Tamer:
    ...
    """
    def __init__(self, env, num_episodes, tame=True, max_objects_in_view=10, max_humans_in_view=10):
