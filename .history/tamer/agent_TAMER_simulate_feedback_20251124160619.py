...
class SGDFunctionApproximator:
    ...
    def __init__(self, env, max_objects_in_view=10, max_humans_in_view=10):
        ...
        for _ in range(env.action_space.shape[0]):
            ...
            model.partial_fit([self.featurize_state(observation_examples[0])], [np.random.uniform(-0.1, 0.1)])
...
class Tamer:
    ...
    def __init__(
        ...
        if model_file_to_load is not None:
            ...
            self.H = SGDRegressor(env, max_objects_in_view, max_humans_in_view)
    ...
    def _get_human_feedback(self, state):
        ...
        for human in self.env.humans:
            ...
            #(f"Human pos: {human['pos']}, direction: {human['direction']}, robot pos: {robot_pos}, distance: {dist}")
