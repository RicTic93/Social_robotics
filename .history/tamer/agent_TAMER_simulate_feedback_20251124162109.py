...
class Tamer:
    ...
    def _get_human_feedback(self, state):
        """
        Feedback automatique basé sur la proxémie :
        - Zone intime     : < 0.45 m → -5
        - Zone personnelle: < 1.2  m → -2
        - Zone sociale    : < 3.0  m → +1
        - Zone publique   : >= 3.0 m → +2
        """
        ...
        for human in self.env.humans:
            human_pos = human["pos"]
            human_dir = human["direction"]

            dist = np.linalg.norm(robot_pos - human_pos)

            # Vérifie si le robot est dans le champ de vision (180°)
            in_fov = self.env._is_robot_in_human_fov(human_pos, human_dir, robot_pos)

            if not in_fov:
                # Hors FOV → feedback neutre
                total_feedback += 0
                continue

            # Zones
            if dist < 0.45:        # Zone intime
                total_feedback += -5.0
            elif dist < 1.2:       # Zone personnelle
                total_feedback += -2.0
            elif dist < 3.0:       # Zone sociale
                total_feedback += +1.0
            else:                  # Zone publique
                total_feedback += +2.0
        else:
            # Si le robot est loin et visible, c'est public, feedback positif léger
            feedback += 1.0
    ...
    def _train_episode(self, episode_index):
        ...
        with open(self.reward_log_path, 'a+', newline='') as write_obj:
            ...
            for ts in count():
                ...
                # Feedback automatique basé sur proxémie
                auto_feedback = self._get_human_feedback(state)
